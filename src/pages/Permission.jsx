...
const Permission = () => {
  ...
  const ToggleSwitch = ({ enabled, onChange }) => {
    ...
    return (
      <label
        ...
        } outline-0 outline-red-300 dark:outline-gray-600 px-1 py-2  rounded-md`}
      >
        <input
          type="checkbox"
          className="sr-only"
        ...
        <div
          className={`w-8 h-5 flex items-center bg-gray-300 rounded-full p-[0.2rem] duration-300 ease-in-out ${
            enabled ? "bg-yellow-400" : "dark:bg-gray-300 "
          }`}
        >
          <div
  ...
  return (
    <>
      <section className="bg-white/80 dark:bg-gray-800 backdrop-blur-md rounded-2xl h-[calc(100vh-62px)]">
        ...
        <div className="bg-white/80 dark:bg-gray-800 backdrop-blur-md rounded-2xl mt-1 p-2 lg:px-2 lg:py-0">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-2.5 lg:gap-1">
            {/* Left Column */}
            <div className="rounded-2xl shadow-md p-2 dark:border-gray-600 border-1">
              <h2 className="text-base  font-medium mb-4 text-black dark:text-white">
                <span className="relative">
                  PERMISSION CONTROL
                  <span className="w-full h-[1px] bottom-[-1.5] block absolute dark:bg-white bg-black" />
                </span>
              </h2>
              <table className="w-full text-sm border-separate border-spacing-y-3">
                ...
                <tbody>
                  {users?.map((user, index) => (
                    <tr
                      key={index}
                      className="bg-gray-50 dark:bg-gray-800  shadow-sm rounded-lg dark:hover:bg-white/10 hover:bg-gray-200 transition duration-200 ease-in-out"
                    >
                      <td
                        className={`py-5 px-1 text-[0.7rem] md:text-sm font-medium text-wrap rounded-l-lg text-black dark:text-white`}
                      >
                        {user?.name}
                      </td>
            ...
            {/* Right Column */}
            <div className="rounded-2xl  shadow-md p-3 lg:p-3 dark:border-gray-600 border-1">
              <h2 className="text-base  font-medium mb-4 text-black dark:text-white">
                <span className="relative">
                  SUMMARY / SETTINGS
                  <span className="w-full h-[1px] bottom-[-1.5] block absolute dark:bg-white bg-black" />
              ...
              <div className="p-2 lg:p-3 text-sm text-black dark:text-white leading-relaxed space-y-2">
                <p>
